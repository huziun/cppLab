(function( $ ) {
    ...
    $.fn.miniEventCalendar = $.fn.MEC = function(options) {
		...
		miniCalendar.on("click touchstart", ".a-date", function(e){
			...
			let elem = $(this);
			let spn =elem.find('span')
			const arr = $('#monthYear').text().split(" ");
			let m = "JanFebMarAprMayJunJulAugSepOctNovDec".indexOf(arr[0]) / 3 + 1 
			console.log(arr[1]+"-"+m+"-"+spn.text());
			calendar.gotoDate(new Date(arr[1],m-1,spn.text()));
		});
